...
export const lambdaRequestHandler = (app, handler) => {
  ...
  return (event, context, ...params) => {
    ...
    const appConfig = app.server || app;
    if (
      (!appConfig.nextConfig.assetPrefix
        || pathPrefixes.indexOf(appConfig.nextConfig.assetPrefix) >= 0)
      && (!appConfig.renderOpts.assetPrefix
        || pathPrefixes.indexOf(appConfig.renderOpts.assetPrefix) >= 0)
    ) {
